// Script para Painel Admin - Nova Verseo com conquistas Customizeveis
...
document.addEventListener('DOMContentLoaded', async function() {
    ...
    if (typeof RouteGuard !== 'undefined') {
        const allowed = await RouteGuard.requireAuth({
            loginMessage: 'Faea login para acessar o painel administrativo',
        ...
        if (!token) {
            window.location.href = 'login.html?message=' + encodeURIComponent('Faea login para acessar o painel administrativo');
            return;
        }
    }
...
function attachEventListeners() {
    ...
    if (createRankForm) {
        createRankForm.addEventListener('submit', async (e) => {
            ...
            if (!name || !color || !description || !customText) {
                showMessage('Preencha todos os campos obrigaterios', 'error');
                return;
            }
    ...
    if (banForm) {
        banForm.addEventListener('submit', async (e) => {
            ...
            if (banType === 'suspension' && !banDays) {
                showMessage('Especifique os dias de suspenseo', 'error');
                return;
            }
            ...
            const confirmed = await confirmCriticalAction({
                ...
                message: `Voce este prestes a ${banDescription}.`,
                keyword,
                confirmText: banType === 'suspension' ? 'Suspender usuerio' : 'Banir usuerio'
            });
            if (!confirmed) {
                showMessage('Operaeeo cancelada', 'error');
                return;
            }
            ...
            try {
                ...
                if (result.success) {
                    showMessage(`Usuerio ${banType === 'suspension' ? 'suspenso' : 'banido'} com sucesso!`, 'success');
    ...
    // Gerenciar permissees de postagem
    ...
    tabBtns.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            // Remove active de todos os botees e conteedos
...
async function loadDefaultRanks() {
    ...
    try {
        ...
        container.innerHTML = '<p style="color: red;">Erro ao carregar conquistas padreo</p>';
    }
}

// ===================== CONQUISTAS CUSTOMIZeVEIS =====================
async function loadCustomRanks() {
    const container = document.getElementById('customRanksContainer');
    try {
        console.log('Carregando conquistas customizeveis...');
        const result = await AuthAPI.getAllCustomRanks();
        console.log('Resultado da API:', result);
        
        if (result.success) {
            if (!result.ranks || result.ranks.length === 0) {
                container.innerHTML = '<p style="text-align: center; color: #999;">Nenhum conquista customizevel criado ainda</p>';
                return;
            }
            console.log('Renderizando', result.ranks.length, 'conquistas customizeveis');
        ...
        console.error('Erro na funeeo loadCustomRanks:', error);
        ...
        <div class="rank-card" data-rank-id="${rank.id}" data-rank-name="${rank.name}" ${rank.backgroundImage ? `style="background-image: linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), url('${rank.backgroundImage}'); background-size: cover; background-position: center;"` : ''}>
            ...
            ` : (!isDefault ? `
                <div class="rank-actions" style="pointer-events: none; opacity: 0.7;">
                    <span style="padding: 5px 10px; font-size: 11px; border-radius: 3px; color: rgba(255,255,255,0.8); border: 1px solid rgba(255,255,255,0.2);">Padreo</span>
                </div>
            ` : '')}
        </div>
    `;
...
function renderRanksList(container, ranks, isDefault) {
    if (!isDefault) {
        // Para customizeveis queremos seeees empilhadas; o grid fica apenas dentro de cada seeeo.
        ...
        // Restaurar comportamento padreo (grid) para os conquistas neo removeveis.
...
async function editRank(rankId, rankName) {
    try {
        ...
        if (!rankData) {
            showMessage('conquista neo encontrado', 'error');
            return;
        }
        ...
        modal.innerHTML = `
            ...
            ">
                ...
                <form id="editRankForm">
                    ...
                    <div style="margin-bottom: 15px;">
                        <label style="color: white; display: block; margin-bottom: 5px;">Descrieeo *</label>
                        <textarea id="editRankDescription" placeholder="Descrieeo" style="width: 100%; height: 70px; padding: 10px; background: rgba(0,0,0,0.3); border: 1px solid #6B46C1; color: white; border-radius: 5px; resize: none;">${rankData.description || ''}</textarea>
                    </div>

                    <div style="margin-bottom: 15px;">
                        <label style="color: white; display: block; margin-bottom: 5px;">Texto Customizevel *</label>
                        <input type="text" id="editRankCustomText" placeholder="Texto customizevel" value="${rankData.customText || ''}" style="width: 100%; padding: 10px; background: rgba(0,0,0,0.3); border: 1px solid #6B46C1; color: white; border-radius: 5px;">
                    </div>
                    ...
                    <div style="margin-bottom: 15px;">
                        ...
                        <small style="color: rgba(255,255,255,0.65); display: block; margin-top: 6px;">Se selecionar arquivo, ele tere prioridade sobre a URL.</small>
                    </div>
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(16, 185, 129, 0.1); border: 1px solid rgba(16, 185, 129, 0.4); border-radius: 5px;">
                        ...
                        <small style="color: rgba(16, 185, 129, 0.85); display: block; margin-top: 8px;">Usuerios com esta conquista podereo acessar o painel de administraeeo com permissees limitadas</small>
                    </div>
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.4); border-radius: 5px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: #3B82F6; font-weight: 600;">
                            ...
                            <span>Permitir Usuerio Escolher (Exibir/Ocultar)</span>
                        </label>
                        <small style="color: rgba(59, 130, 246, 0.8); display: block; margin-top: 8px;">Marcado: usuerio pode escolher exibir ou neo a conquista. Desmarcado: conquista fica fixa nas conquistas</small>
                    </div>
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(236, 72, 153, 0.1); border: 1px solid rgba(236, 72, 153, 0.4); border-radius: 5px;">
                        ...
                        <small style="color: rgba(236, 72, 153, 0.85); display: block; margin-top: 8px;">Usuerios com esta conquista podereo editar e excluir seus preprios downloads e tambem os de outros usuerios</small>
                    </div>
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(245, 158, 11, 0.1); border: 1px solid rgba(245, 158, 11, 0.4); border-radius: 5px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: #F59E0B; font-weight: 600;">
                            ...
                            <span>Permitir Usuerio Ordenar Todos os conquistas (incluindo fixos)</span>
                        </label>
                        <small style="color: rgba(245, 158, 11, 0.85); display: block; margin-top: 8px;">Com esta conquista, o usuerio pode reordenar tambem conquistas fixas no perfil.</small>
                    </div>
                    ...
                    <div style="margin: 20px 0; padding: 15px; background: rgba(56, 189, 248, 0.1); border: 1px solid rgba(56, 189, 248, 0.45); border-radius: 5px;">
                        <label style="display: flex; align-items: center; gap: 10px; cursor: pointer; color: #38BDF8; font-weight: 600;">
                            ...
                            <span>Permitir usuerio tenha Card animado</span>
                        </label>
                        <small style="color: rgba(56, 189, 248, 0.85); display: block; margin-top: 8px;">Mostra o boteo "Perfil Animado" no perfil do usuerio.</small>
                    </div>
...
async function loadUsers() {
    ...
    try {
        ...
        if (!result.success || !result.users || result.users.length === 0) {
            cachedUsers = [];
            currentUsersPage = 1;
            container.innerHTML = '<p>Nenhum usuerio encontrado</p>';
            return;
        }
        ...
        cachedUsers = [];
        currentUsersPage = 1;
        container.innerHTML = '<p style="color: red;">Erro ao carregar usuerios: ' + error.message + '</p>';
    }
}
...
function renderUsersPage(container) {
    ...
    controls.innerHTML = `
        ...
        <span class="users-page-label">Pegina ${currentUsersPage} de ${totalPages}</span>
        <button type="button" class="users-page-btn" data-move="next" ${currentUsersPage === totalPages ? 'disabled' : ''}>Prexima</button>
    `;
            ...
            <table class="users-table">
                <thead>
                    <tr>
                        ...
                        <th>Nickname</th>
                        <th>E-mail</th>
                        <th>conquistas Customizeveis</th>
                        <th>Ordenar Todos</th>
                        <th>Aeees</th>
                    </tr>
                </thead>
...
function renderUserRankBadge(userId, rank) {
    return `
        <span class="user-rank-badge" style="background-color: ${rank.color}40; border-color: ${rank.color}; color: ${rank.color};" data-user-id="${userId}" data-rank-id="${rank.id}">
            ...
            <button class="remove-rank-btn" style="background: none; border: none; color: ${rank.color}; cursor: pointer; padding: 0; margin-left: 4px; font-weight: bold;">e</button>
        </span>
    `;
}
...
async function loadUserRanks(userId) {
    try {
        ...
        if (result.success && result.ranks && result.ranks.length > 0) {
            ...
            container.innerHTML = '<p style="color: #999; font-size: 12px;">Nenhum conquista atribuedo</p>';
        }
    } catch (error) {
        console.error('Erro ao carregar conquistas do usuerio:', error);
    }
}
...
async function loadRankSelectOptions(userId) {
    try {
        ...
        console.error('Erro ao carregar opeees de conquistas:', error);
    }
}
...
async function addUserRank(userId) {
    ...
    try {
        ...
        if (result.success) {
            showMessage('conquista atribuedo com sucesso!', 'success');
...
async function removeUserRank(userId, customRankId) {
    if (!confirm('Tem certeza que deseja remover este conquista do usuerio?')) {
        return;
    }
...
async function deleteUserAccount(userId) {
    if (!canRevealEmails) {
        showMessage('Somente o dono da conta pode excluir usuerios', 'error');
        return;
    }

    const normalizedId = Number(userId);
    if (!Number.isInteger(normalizedId) || normalizedId <= 0) {
        showMessage('ID de usuerio invelido', 'error');
        return;
    }
    ...
    const confirmed = await confirmCriticalAction({
        title: 'Excluir conta',
        message: `Tem certeza que deseja excluir completamente a conta de ${label}? Esta aeeo remove perfil, conquistas, downloads e histerico relacionado.`,
    ...
    try {
        ...
        if (result.success) {
            showMessage(result.message || 'Conta exclueda com sucesso!', 'success');
            await loadUsers();
            loadBans();
            loadPostPermissions();
        } else {
...
async function confirmCriticalAction(options = {}) {
    ...
    return showKeywordConfirm(
        'Confirmaeeo adicional',
        'Aeeo cretica detectada.',
        normalizedKeyword
    );
}
...
async function loadBans() {
    ...
    try {
        ...
        if (result.success && result.bans && result.bans.length > 0) {
                    ...
                    <div class="ban-card" data-ban-id="${ban.id}">
                        <div class="ban-info">
                            ...
                            <div class="ban-details">
                                <strong>Tipo:</strong> ${ban.banType === 'suspension' ? 'Suspenseo Temporeria' : 'Ban Permanente'} ${isExpired ? '(Expirado)' : ''}
                            </div>
                            <div class="ban-details">
            ...
            container.innerHTML = '<p style="text-align: center; color: #999;">Nenhum usuerio banido</p>';
        }
    } catch (error) {
...
async function unbanUser(banId) {
    ...
    try {
        ...
        if (result.success) {
            showMessage('Usuerio desbanido com sucesso!', 'success');
            loadBans();
        } else {
            showMessage('Erro: ' + result.message, 'error');
        }
...
async function loadPostPermissions() {
    ...
    try {
        ...
        if (result.success && users.length > 0) {
            ...
            container.innerHTML = '<p style="text-align: center; color: #999;">Nenhum usuerio com permissees individuais</p>';
            container.style.display = 'block';
            table.style.display = 'none';
        }
    } catch (error) {
        container.innerHTML = '<p style="color: red;">Erro ao carregar permissees</p>';
        console.error('Erro ao carregar permissees:', error);
    }
}
                ...
                <div class="rank-card" style="flex-direction: column; align-items: flex-start; background: linear-gradient(135deg, ${rank.color}22, ${rank.color}11);">
                    <div style="display: flex; align-items: center; gap: 10px; width: 100%; margin-bottom: 10px;">
                        ...
                        <div>
                            ...
                            <p style="margin: 0; font-size: 12px; color: rgba(255, 255, 255, 0.6);">Nevel ${rank.rankLevel}/10</p>
                        </div>
                    </div>
                    <p style="margin: 0; font-size: 12px; color: rgba(255, 255, 255, 0.7); width: 100%;">
                        <strong>${rank.minPostages} postagens</strong> necesserias
                    </p>
...
async function saveIndividualPermissions(username) {
    ...
    try {
        ...
        if (result.success) {
            showMessage(`Permissees salvas para ${username}!`, 'success');
        ...
        showMessage('Erro ao salvar permissees: ' + error.message, 'error');
    }
}

async function clearIndividualPermissions(username) {
    if (!confirm(`Tem certeza que deseja revogar todas as permissees de ${username}?`)) {
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            showMessage(`Permissees removidas de ${username}!`, 'success');
            loadPostPermissions();
        } else {
            showMessage('Erro: ' + result.message, 'error');
        }
    } catch (error) {
        showMessage('Erro ao remover permissees: ' + error.message, 'error');
    }
}
